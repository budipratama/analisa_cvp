...
class DbManager extends BaseManager
{
    ...
    {
        $string = $this->itemTable;
        if(Yii::$app->user->usermode == 'admin'){
            if($string == "{{%auth_item}}" && $type == '2'){
                $query = (new Query)
                ->from($this->itemTable)
                ->where(['type' => $type])
                ->andWhere(['not like','name','/gii/'])
                ->andWhere(['not like','name','/admin/assignment/'])
                ->andWhere(['not like','name','/admin/permission/'])
                ->andWhere(['not like','name','/admin/route/'])
                ->andWhere(['not like','name','/admin/rule/'])
                ->andWhere(['not like','name','/debug/'])
                ->andWhere(['not in','name',['/admin/*']])
                ->andWhere(['not in','name',['/admin/menu/*']])
                ->andWhere(['not in','name',['/admin/menu/create']])
                ->andWhere(['not in','name',['/admin/menu/delete']])
                ->andWhere(['not like','name','/admin/default/'])
                ->andWhere(['like','name','/']);
            }
            else if($string = "{{%auth_item}}" && $type == '1'){
                $query = (new Query)
                ->from($this->itemTable)
                ->where(['type' => $type])
                ->andWhere(['not in','name','developer']);
            }
            else{
                $query = (new Query)
                ->from($this->itemTable)
