...
/* @var $searchModel backend\models\FixedCostSearch */
...
<div class="fixed-cost-index">
    ...
    <?php Pjax::begin(['id'=>'fixed-cost-grid','enablePushState'=>false]);?>
    <?php 
...
$js = <<<JS
    ...
    $('form#{$searchModel->formName()}').on('beforeSubmit',function(e){
        ...
        var \$form = $(this);
        $.pjax.reload({url:\$form.attr('action')+'&per-page='+$("input[name='per-page']").val()+'&'+\$form.serialize(),container:"#fixed-cost-grid",async:false,replace:false});                 
        return false;
    });
