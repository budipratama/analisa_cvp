...
/* @var $searchModel backend\models\AntrianSearch */
...
<div class="antrian-index">
    ...
    <?php Pjax::begin(['id'=>'antrian-grid','enablePushState'=>false]);?>
    <?= GridView::widget([
...
$js = <<<JS
    ...
    $('form#{$searchModel->formName()}').on('beforeSubmit',function(e){
        ...
        var \$form = $(this);
        $.pjax.reload({url:\$form.attr('action')+'&per-page='+$("input[name='per-page']").val()+'&'+\$form.serialize(),container:"#antrian-grid",async:false,replace:false});                 
        return false;
    });
