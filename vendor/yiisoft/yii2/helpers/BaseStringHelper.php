...
class BaseStringHelper
{
    ...
    public function hideColumn($word)
    {
        $words_hidding = array("first_ip","last_ip","first_user","last_user","first_update","last_update");
     
        $hasil = 0;
         
        $count_word = count($words_hidding);
         
        for ($i=0;$i<$count_word;$i++)
        {     
            if (stristr($word,$words_hidding[$i]))
                $hasil=1; 
        }
