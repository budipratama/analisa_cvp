    ...
    public static function getAssignedMenu($userId, $root = null, $callback = null, $refresh = false)
    {
        ...
        if (Yii::$app->user->isGuest) {
            ...
            if(Yii::$app->user->usermode == "admin"){
                $menus = Menu::find()->asArray()->where(['NOT IN','name',["Assignments","Permission","Routes","Rules"]])->indexBy('id')->all();
            }
            else{
                $menus = Menu::find()->asArray()->indexBy('id')->all();
            }
            ...
            if (count($filter1)) {
               /* $query->where('route like :filter');
                ...
                if(Yii::$app->user->usermode == 'admin'){
                    foreach($filter1 as $filter){
                        if($filter == '/%'){
                           $query->where('route not like :f1 and route not like :f2 and route not like :f3 and route not like :f4');
                           $assigned = array_merge($assigned, $query->params([':f1' => '/admin/assignment%',':f2' => '/admin/permission%',':f3' => '/admin/route%',':f4' => '/admin/rule%'])->column());
                        } 
                        else{
                            $query->where('route like :filter');
                            foreach ($filter1 as $filter) {
        ...
        foreach ($assigned as $id) {
            ...
            if ($menu['parent'] == $parent) {
                ...
                if ($callback !== null) {
                    ...
                    else if ($menu['name']=='Configuration') {
                        $item = [
                            'label' => $menu['name'],
                            'url' => static::parseRoute($menu['route']),
