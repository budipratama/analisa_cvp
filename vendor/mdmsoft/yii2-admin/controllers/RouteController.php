...
use backend\controllers\CosController;
...
class RouteController extends \yii\web\Controller
{
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->getRequest()->post())) {
            // $obj = $model->route;
            // $a = "\backend\controllers\\".$model->route;//.'(1,\'CallSummary\')';
            // $controller = strtolower(implode("-",preg_split('/(?=[A-Z])/', str_replace("Controller","",$model->route), -1, PREG_SPLIT_NO_EMPTY)));
            // $reflection = new \ReflectionClass(new $a);
            // $methods = array();
            // foreach ($reflection->getMethods() as $m) {
            //     if ($m->class == "backend\controllers\\".$model->route) {
            //         $methods[] = $m->name;
            //         // jika fungsi mengandung karakter action
            //             if (preg_match('/^action/', $m->name)) {

            //                 // echo $value." masuk bro<br>";
            //                 $makan = preg_split('/(?=[A-Z])/', $m->name, -1, PREG_SPLIT_NO_EMPTY);
            //                 unset($makan[0]);
            //                 // print_r($makan);
            //                 $routes[] = $controller.'/'.strtolower(implode("-",$makan))."<br>";
            //             }
            //     }
            // }
